...
class DetLocalVisualizer(Visualizer):
    ...
    def __init__(self,
        ...
        super().__init__(
            ...
            save_dir=save_dir)
        self.save_dir = save_dir
        self.file_format = file_format  # 保存
        self.bbox_color = bbox_color
        self.text_color = text_color
        self.mask_color = mask_color
        self.line_width = line_width
    ...
    def add_datasample(
        ...
        if out_file is None:
            if self.save_dir is None:
                raise ValueError("save_dir is not set in visualizer, cannot save image")

            # 取原始图像路径
            img_path = data_sample.metainfo.get('img_path', 'unnamed')
            # 获取原文件名（包含后缀）
            base_name = os.path.basename(img_path)
            # 保存路径
            out_file = os.path.join(self.save_dir, base_name)
